...
function matchInputFieldDirective(
...
function matchArgumentDirective(
  definition: DirectiveDefinitionNode,
  directive: DirectiveNode,
  node: InputValueDefinitionNode
) {
...
function matchEnumValueDirective(
  definition: DirectiveDefinitionNode,
  directive: DirectiveNode,
...
type TypeDefinitionOrExtension = TypeDefinitionNode | TypeExtensionNode | SchemaDefinitionNode
...
export class GraphQLTransform {
   ...
   */
  public transform = ({ typeDefs, resolvers = {} }: { typeDefs: string; resolvers?: Record<string, any> }) => {
    this.seenTransformations = {}
    const context = new TransformerContext(typeDefs)
      ...
      Object.keys(resolvers[typeName]).forEach(fieldName => {
        context.setResource(
          ...
          createResolver({
            ...
            resolver: resolvers[typeName][fieldName],
          })
        )
      })
    })
    ...
    let allModelDefinitions = [...context.document.definitions]
    ...
    for (const transformer of this.transformers) {
      ...
      for (const def of context.document.definitions as TypeDefinitionOrExtension[]) {
        switch (def.kind) {
          case 'SchemaDefinition':
            this.transformSchema(transformer, def, validDirectiveNameMap, context)
            break
  ...
  private transformSchema(
    ...
    for (const dir of def.directives || []) {
      ...
      if (matchDirective(transformer.directive, dir, def)) {
        if (isFunction(transformer.schema)) {
          const transformKey = 'schema'
          if (!this.seenTransformations[transformKey]) {
            transformer.schema(def, dir, context)
            this.seenTransformations[transformKey] = true
          }
        } else {
          throw new InvalidTransformerError(
            `The transformer '${transformer.name}' must implement the 'schema()' method`
          )
        }
      }
    }
    ...
    for (const dir of def.directives || []) {
      ...
      if (matchDirective(transformer.directive, dir, def)) {
        if (isFunction(transformer.object)) {
          ...
          throw new InvalidTransformerError(
            `The transformer '${transformer.name}' must implement the 'object()' method`
          )
        }
      }
      index++
    ...
    for (const dir of arg.directives || []) {
      ...
      if (matchArgumentDirective(transformer.directive, dir, arg)) {
        if (isFunction(transformer.argument)) {
          ...
          throw new InvalidTransformerError(
            `The transformer '${transformer.name}' must implement the 'argument()' method`
          )
        }
      }
      index++
    ...
    for (const dir of def.directives || []) {
      ...
      if (matchDirective(transformer.directive, dir, def)) {
        if (isFunction(transformer.interface)) {
          ...
          throw new InvalidTransformerError(
            `The transformer '${transformer.name}' must implement the 'interface()' method`
          )
        }
      }
      index++
    ...
    for (const dir of def.directives || []) {
      ...
      if (matchDirective(transformer.directive, dir, def)) {
        if (isFunction(transformer.scalar)) {
          ...
          throw new InvalidTransformerError(
            `The transformer '${transformer.name}' must implement the 'scalar()' method`
          )
        }
      }
      index++
    ...
    for (const dir of def.directives || []) {
      ...
      if (matchEnumValueDirective(transformer.directive, dir, def)) {
        if (isFunction(transformer.enumValue)) {
          ...
          throw new InvalidTransformerError(
            `The transformer '${transformer.name}' must implement the 'enumValue()' method`
          )
        }
      }
      index++
    ...
    for (const dir of def.directives || []) {
      ...
      if (matchInputFieldDirective(transformer.directive, dir, def)) {
        if (isFunction(transformer.inputValue)) {
          ...
          throw new InvalidTransformerError(
            `The transformer '${transformer.name}' must implement the 'inputValue()' method`
          )
        }
      }
      index++
